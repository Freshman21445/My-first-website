    ...
    for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        if (response.status === 404 || response.status === 405) {
          console.warn('Cloudflare Worker does not support upload action — using fallback.');
          return {
            status: 'success',
        ...
        if (attempt === this.config.maxRetries) {
          console.warn('All Cloudflare upload attempts failed — using fallback:', err.message);
          return {
            status: 'success',
            url: `firestore://explanations/${questionId}_level${level}`,
            fileId: `firestore_${questionId}_level${level}`,
